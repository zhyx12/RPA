...
class roibatchLoader(data.Dataset):
    def __init__(self, roidb, ratio_list, ratio_index, batch_size, num_classes, training=True, normalize=None,
                 seg_return=False, path_return=False):
        self._roidb = roidb
    ...
    def __getitem__(self, index):
        ...
        if self.training:
            ...
            if self._roidb[index_ratio]['need_crop']:
                if ratio < 1:
                    ...
                    if gt_boxes.numel():
                        ...
                        x_s = np.random.choice(range(data_width - trim_size))
                        data = data[:, :, x_s:(x_s + trim_size), :]

            # based on the ratio, padding the image.
            data_height, data_width = data[0].size(0), data[0].size(1)
            # TODO: 检查这一修改是否有必要
            # https://github.com/jwyang/faster-rcnn.pytorch/issues/629
            if ratio < 1:
            ...
            return padding_data, im_info, gt_boxes_padding, num_boxes, blobs['image']
        else:
            ...
            return data, im_info, gt_boxes, num_boxes, blobs['image']
