...
def _smooth_l1_loss(bbox_pred, bbox_targets, bbox_inside_weights, bbox_outside_weights, sigma=1.0, dim=[1], mean=True):
    sigma_2 = sigma ** 2
...
def compare_grid_sample():
    ...
    input = torch.randn(N, C, H, W, requires_grad=True).cuda()
    input_p = input.clone().data.contiguous()

    grid = torch.randn(N, H, W, 2, requires_grad=True).cuda()
    grid_clone = grid.clone().contiguous()

    out_offcial = F.grid_sample(input, grid)
    grad_outputs = torch.rand(out_offcial.size()).cuda()
